...
del /Q /S *111 2>nul
del /Q RuoYi-Vue3-FastAPI\ruoyi-fastapi-backend\module_automation\controller\task_controller.py.bak 2>nul
del /Q RuoYi-Vue3-FastAPI\ruoyi-fastapi-frontend\src\views\automation\task\index.vue111 2>nul
echo ✅ 完成
...
echo.
echo [6/7] 清理 Node.js 缓存（保留 node_modules）...
...
rd /s /q RuoYi-Vue3-FastAPI\ruoyi-fastapi-backend\static\profile 2>nul
echo ✅ 完成
...
echo 📋 保留的重要文件:
echo    ✓ 源代码文件
echo    ✓ 配置文件 (.env.example, config.example.json)
echo    ✓ 数据库脚本 (*.sql)
echo    ✓ 启动脚本 (start_*.bat)
echo    ✓ 核心文档 (README.md, QUICK_START.md)
echo    ✓ node_modules (如需删除请手动执行)
echo.
echo 💡 下一步:
