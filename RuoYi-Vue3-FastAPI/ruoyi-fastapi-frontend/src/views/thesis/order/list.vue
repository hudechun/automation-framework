      ...
      >
        ...
        <div class="order-actions">
          ...
          <button
            ...
            @click="handleMockPay(order)"
            title="模拟支付成功（自动完成回调，仅管理员）"
          >
            <Zap :size="16" />
            <span>模拟支付</span>
          </button>
...
import { mockPayment } from '@/api/thesis/payment'
...
const handleMockPay = (order) => {
  ElMessageBox.confirm(
    `确定要模拟支付订单 ${order.orderNo} 吗？<br><br>
    <strong style="color: #f59e0b;">⚠️ 这是开发调试功能</strong><br>
    将自动完成支付并激活套餐（无感知回调）`,
    '模拟支付确认',
    {
      type: 'warning',
    ...
    try {
      const res = await mockPayment(order.orderId)
      ElMessage.success({
        message: `模拟支付成功！交易号: ${res.data.transaction_no}`,
        duration: 3000
      })
      getList()
    } catch (error) {
      ElMessage.error('模拟支付失败: ' + (error.message || '未知错误'))
    }
  })
}
