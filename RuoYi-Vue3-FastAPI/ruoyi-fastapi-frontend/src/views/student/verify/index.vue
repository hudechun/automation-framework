      ...
      <div v-if="result" class="result-area">
        ...
        <template v-else>
          ...
          <div v-if="result.reportImageUrl" class="report-wrap">
            <p class="report-label">学籍报告图</p>
            <img :src="result.reportImageUrl" alt="学籍报告" class="report-img" @error="onImgError" />
...
<script setup name="VerifyPage">
import { ref, onMounted } from 'vue'
import { useRoute } from 'vue-router'
import { Loading } from '@element-plus/icons-vue'
import { getVerifyCheck } from '@/api/student/verification'
...
function handleCheck(isFromUrl = false) {
  ...
  getVerifyCheck(c)
    .then((res) => {
      ...
      if (canRedirect) {
        const url = reportUrl || `${import.meta.env.VITE_APP_BASE_API || '/dev-api'}/verify/report/image?code=${encodeURIComponent(c)}`
