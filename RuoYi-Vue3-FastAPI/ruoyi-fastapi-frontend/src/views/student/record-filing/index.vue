<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="warning" plain icon="Document" @click="handleBatchDownloadReport">批量下载备案表</el-button>
      </el-col>
    ...
    <el-table ref="tableRef" v-loading="loading" :data="studentList" border @selection-change="handleSelectionChange" row-key="id">
      ...
      <el-table-column label="专业" align="center" prop="major" min-width="80" show-overflow-tooltip />
      <el-table-column label="层次" align="center" prop="level" width="70" />
      <el-table-column label="证书编号" align="center" prop="certificateNo" width="140" show-overflow-tooltip />
    ...
    <!-- 导入对话框 -->
    <el-dialog title="Excel 导入备案表学生" v-model="importVisible" width="460px" append-to-body>
      <el-upload
        ref="uploadRef"
        :limit="1"
        accept=".xlsx,.xls"
      ...
      >
        ...
        <template #tip>
          <div class="el-upload__tip">表头需含：姓名、性别、出生日期、入学日期、毕（结）业日期、学校名称、专业、学制、层次、学历类别、学习形式、毕（结）业、证书编号、校（院）长姓名、验证有效日期；照片可为内嵌图片。</div>
        </template>
      </el-upload>
      <template #footer>
    ...
    <!-- 编辑 -->
    <el-dialog title="编辑备案表记录" v-model="editVisible" width="720px" append-to-body destroy-on-close>
      <el-form ref="editFormRef" :model="editForm" :rules="editRules" label-width="120px">
        <el-row :gutter="16">
          ...
          <el-col :span="12">
            <el-form-item label="毕（结）业日期" prop="graduationDate">
              <el-date-picker v-model="editForm.graduationDate" type="date" value-format="YYYY-MM-DD" placeholder="毕结业日期" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="毕（结）业" prop="graduationStatus">
              <el-input v-model="editForm.graduationStatus" placeholder="毕业/结业" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
          ...
          <el-col :span="12">
            <el-form-item label="证书编号" prop="certificateNo">
              <el-input v-model="editForm.certificateNo" placeholder="证书编号" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="校（院）长姓名" prop="presidentName">
              <el-input v-model="editForm.presidentName" placeholder="校院长姓名" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
    ...
    <!-- 报告图预览 -->
    <el-dialog v-model="reportVisible" title="学历备案表报告图" width="80%" append-to-body destroy-on-close class="report-dialog">
...
<script setup name="StudentRecordFiling">
import { listStudent, updateStudent, importStudents, uploadPhoto, getReportImageBlob, getQrImageBlob, batchDownloadQr, batchDownloadReport } from '@/api/student/recordFiling'
...
function handleBatchDownloadQr() {
  ...
  if (!ids || ids.length === 0) {
    proxy.$modal.msgWarning('请先勾选要下载的记录')
    return
  }
  batchDownloadQr(ids)
    .then((blob) => {
      saveAs(blob, '学历备案表二维码.zip')
...
function handleBatchDownloadReport() {
  ...
  if (ids.length > 50) {
    proxy.$modal.msgWarning('单次最多选择 50 条')
    return
  }
  proxy.$modal.loading('正在生成备案表，请稍候…')
  batchDownloadReport(ids)
    .then(async (blob) => {
      ...
      saveAs(blob, '学历备案表报告.zip')
      proxy.$modal.msgSuccess(`已下载 ${ids.length} 个备案表报告`)
    })
    .catch((err) => {
      const msg = err?.response?.data?.msg || err?.msg || err?.message || '批量下载失败'
      proxy.$modal.msgError(typeof msg === 'string' ? msg : '批量下载失败')
    })
    .finally(() => {
...
function submitEdit() {
  editFormRef.value?.validate((valid) => {
    ...
    const payload = {
      ...
      graduationStatus: editForm.value.graduationStatus || undefined,
      ...
      certificateNo: editForm.value.certificateNo || undefined,
      presidentName: editForm.value.presidentName || undefined,
