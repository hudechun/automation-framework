...
ai_model_controller = APIRouterPro(
    prefix='/system/ai-model', order_num=1, tags=['系统管理-AI模型配置'], dependencies=[PreAuthDependency()]
)
...
@ai_model_controller.get(
    ...
    dependencies=[UserInterfaceAuthDependency('system:ai-model:list')],
)
...
@ai_model_controller.get(
    '/preset-models',
    summary='获取预设模型列表',
    description='获取预设模型列表（用于下拉选择）',
    response_model=DataResponseModel[list[PresetModelModel]],
)
async def get_preset_models(
    ...
    """获取预设模型列表"""
    result = await AiModelService.get_preset_models(query_db, model_type, provider)
    logger.info(f'获取预设模型列表成功，类型: {model_type}, 提供商: {provider}')
    return ResponseUtil.success(data=result)


@ai_model_controller.get(
    '/models-by-type/{model_type}',
    summary='根据类型获取模型列表',
    description='根据模型类型获取所有启用的模型',
    response_model=DataResponseModel[list[AiModelConfigModel]],
)
async def get_models_by_type(
    request: Request,
    model_type: Annotated[str, Path(description='模型类型（language/vision）')],
    query_db: Annotated[AsyncSession, DBSessionDependency()],
) -> Response:
    """根据类型获取模型列表"""
    result = await AiModelService.get_models_by_type(query_db, model_type)
    logger.info(f'获取{model_type}类型模型列表成功')
    return ResponseUtil.success(data=result)
...
@ai_model_controller.get(
    ...
    dependencies=[UserInterfaceAuthDependency('system:ai-model:query')],
)
...
async def get_default_config(
    request: Request,
    model_type: str = Query('language', description='模型类型（language/vision）'),
    query_db: Annotated[AsyncSession, DBSessionDependency()] = None,
) -> Response:
    """获取默认AI模型配置"""
    result = await AiModelService.get_default_config(query_db, model_type)
    if not result:
        return ResponseUtil.error(msg=f'未设置默认{model_type}模型')
    logger.info(f'获取默认{model_type}模型配置成功')
    return ResponseUtil.success(data=result)
...
async def get_enabled_configs(
    ...
    """获取启用的AI模型配置列表"""
    result = await AiModelService.get_enabled_configs(query_db, model_type)
...
@ai_model_controller.post(
    ...
    dependencies=[UserInterfaceAuthDependency('system:ai-model:add')],
)
...
@ai_model_controller.put(
    ...
    response_model=ResponseBaseModel,
    dependencies=[UserInterfaceAuthDependency('system:ai-model:edit')],
)
...
@ai_model_controller.delete(
    ...
    response_model=ResponseBaseModel,
    dependencies=[UserInterfaceAuthDependency('system:ai-model:remove')],
)
...
@ai_model_controller.post(
    ...
    dependencies=[UserInterfaceAuthDependency('system:ai-model:test')],
)
